...
export default function Sources() {
  ...
  const handleDelete = async (source: SourceItem) => {
    if (!confirm(`¿Eliminar la fuente "${source.name}"? Esto también borra sus publicaciones asociadas si las hubiera.`)) return;
    try {
                      ...
                      <TableRow key={source.id}>
                        ...
                        <TableCell className="px-4 py-3 text-start text-theme-sm">
                          <div className="flex gap-3">
                            ...
                            <button className="text-error-500 hover:underline" onClick={() => handleDelete(source)}>
                              Eliminar
                            </button>
                          </div>
                        </TableCell>
