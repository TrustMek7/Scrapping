...
import { extractFacebookPost, extractLatestFeedPost } from "@/lib/facebook/extractor";
import {
  hasStoredSession,
...
export async function getLatestPagePost(input: unknown) {
  ...
  return withFacebookContext<FacebookPost>(true, async (context) => {
    ...
    const post = await extractLatestFeedPost(page);
    if (isFacebookVideoUrl(post.url)) {
      throw new FacebookError(
        "INVALID_URL",
        "La publicación más reciente es un video — los videos no forman parte de este piloto.",
        400,
      );
    }
    return post;
